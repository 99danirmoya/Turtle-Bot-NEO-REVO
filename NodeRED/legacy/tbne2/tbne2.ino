...
float temp, hum, alt, pres, vbat, lat, lon, veloc;                                                                                          // Definicion de las variables 'float'
...
void loop(){
  ...
  if((millis() - lastTime) > TIMER_DELAY){                                                                                                  // Send an MQTT publish request every 5 secs
    if(WiFi.status()== WL_CONNECTED){                                                                                                       // Check WiFi connection status
      ...
      // =====================================================================================================================================================
      char dataStr[100];                                                                                                                    // Se crea un string de caracteres para guardar ambas medidas, se reservan 60 espacios
      ...
      sprintf(dataStr, "%5.2f, %6.2f, %4.2f, %4.2f, %4.3f, %10.6f, %10.6f, %1d, %5.2f", temp, pres, alt, hum, vbat, lat, lon, sat, veloc);  // La funcion 'sprintf' de C++ se usa para poder introducir las medidas y elegir el ancho de numero y su precision. Las medidas se separan con una coma ','
...
void callback(char* topic, byte* message, unsigned int length){                                                                             // Funcion que recibe el topico MQTT, el mensaje y la longitud del mismo
  ...
  if(String(topic) == "moya/luces"){                                                                                                        // If a message is received on the topic esp32/output, you check if the message is either "on" or "off". 
    Serial.print("Changing output to ");                                                                                                    // Cambien el estado del output dependiendo del mensaje enviado en el topico de suscripcion
    if(messageTemp == "on"){
